...
Чтобы передать файл с компьютера Б (MacOS) на компьютер С (MacOS) через компьютер А (Windows), используя терминал на компьютере А, мы воспользовались командой ``` scp ```  для копирования файла через SSH.
 
Для выполнения лабораторной работы мы сделали:
...
#### Выполнение лабораторной работы:
Мы решили обернуть в docker-контейнер пито, написанный на языке Python. Были созданы файлы dockerfile: bad и Good , в которых были описаны инструкции для Docker-a.
...
COPY 11.txt /app/11.txt
WORKDIR /app
RUN pip install --no-cache-dir -r 11.txt
...
<img src="https://github.com/nttrg/oblaka/blob/main/screens/scrren5.jpeg" >
